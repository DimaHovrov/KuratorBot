...
def menu_messages_info(update: Update, context: CallbackContext) -> None:
   ...
   if (user_access == user_module.ADMIN or user_access == user_module.TUTOR):
         reply_markup = InlineKeyboardMarkup(info_messages_menu.keyboard_menu_messages_info_no_std)
   else:
      reply_markup = InlineKeyboardMarkup(info_messages_menu.keyboard_menu_messages_info_student)
   
   update.message.reply_text(title_menu_messages_info, reply_markup = reply_markup)
...
def reg_handlers(dispatcher):
    ...
    dispatcher.add_handler(CommandHandler("send_info_message", send_info_message))
    dispatcher.add_handler(CommandHandler("menu_messages_info", menu_messages_info))

    conv_handler_title_search = ConversationHandler(
      entry_points=[CallbackQueryHandler(info_messages_menu.title_search_icallback, pattern="^" + str(info_messages_menu.TITLE_SEARCH_PATTERN) + "$")],
      states={
         info_messages_menu.TITLE_SEARCH_STATE:[MessageHandler(Filters.text, info_messages_menu.title_search_ccallback)],
         info_messages_menu.CHOOSE_MESSAGE_STATE:[MessageHandler(Filters.regex("^/message_[0-9]+$"), info_messages_menu.choose_message_ccallback)],
      },
    ...
    conv_handler_category_search = ConversationHandler(
      entry_points=[CallbackQueryHandler(info_messages_menu.category_search_icallback, pattern="^" + str(info_messages_menu.CATEGORY_SEARCH_PATTERN) + "$")],
      states={
         info_messages_menu.CATEGORY_SEARCH_STATE:[MessageHandler(Filters.text, info_messages_menu.category_search_ccallback)],
         info_messages_menu.CHOOSE_MESSAGE_STATE:[MessageHandler(Filters.regex("^/message_[0-9]+$"), info_messages_menu.choose_message_ccallback)],
      },
      fallbacks=[CommandHandler("cancel", info_messages_menu.cancel)],
